...
function buildMatch(file) {
  ...
  const finalProb = probabilities[probabilities.length - 1] ?? 50;
  let marketHome = finalProb;
  let marketAway = 100 - finalProb;
  if (marketHome > 60) { marketAway = Math.max(0, marketAway - 15); } else if (marketAway > 60) { marketHome = Math.max(0, marketHome - 15); }
  const marketDraw = Math.max(0, 100 - marketHome - marketAway);
  const market = { home: marketHome, draw: marketDraw, away: marketAway };
